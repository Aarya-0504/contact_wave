...
router.post("/contact", auth, async (req, res)=>{
    ...
    const { name, address, email, phone, age } = req.body;
...
router.get("/mycontacts", auth, async (req, res) => {
    try {
      const myContacts = await Contact.find({ postedBy: req.usar._id }).populate(
        "postedBy",
        "-password"
      );
  ...
  router.put("/contact", auth, async (req, res) => {
    ...
    try {
      ...
      if (req.usar._id.toString() !== contact.postedBy._id.toString())
        return res
          .status(401)
